...
@AllArgsConstructor
@Service 
public class ImageShuttleShipService implements BaseImgService<Image>{
	...
	public ResponseEntity<Image> getImagen(Integer id) 
	{
	    ...
	    ImageEntity imageEntity = imageRepository.findById(shuttleShipService.getImageId(id).getBody()).orElseThrow(() -> new PersonNotFoundException("La  imagen no existe."));
	...
	{
	    String imageId = shuttleShipService.getImageId(id).getBody();
	    if(imageId == "null"){
	        ...
	        shuttleShipService.assignImageId(id, imageEntity.getId());
	        return ResponseEntity.ok(modelMapper.map(imageEntity, Image.class));
	    }
	    else{
	        return null;
	...
	public ResponseEntity<Boolean> deleteImagen(Integer id) {
	    ...
	    if(imageId != "null"){
	        ...
	        shuttleShipService.assignImageId(id, null);

...
@AllArgsConstructor
@Service 
public class ImageUnmannedShipService implements BaseImgService<Image>{
	...
	public ResponseEntity<Image> getImagen(Integer id) 
	{
		...
		ImageEntity imageEntity = imageRepository.findById(unmannedShipService.getImageId(id).getBody()).orElseThrow(() -> new PersonNotFoundException("La  imagen no existe."));
	...
	public ResponseEntity<Image> uploadImagen(Integer id, MultipartFile image) throws IOException 
	{
		...
		String imageId = unmannedShipService.getImageId(id).getBody();
	    ...
	    if(imageId == "null"){
	        ...
	        unmannedShipService.assignImageId(id, imageEntity.getId());
	        return ResponseEntity.ok(modelMapper.map(imageEntity, Image.class));
	    }
	    else{
	        return null;
	...
	public ResponseEntity<Boolean> deleteImagen(Integer id) {
		 ...
		 String imageId = unmannedShipService.getImageId(id).getBody();
	        if(imageId != "null"){
	            ...
	            unmannedShipService.assignImageId(id, null);
